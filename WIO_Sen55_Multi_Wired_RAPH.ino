// todo add Enable / Disable static choice for each sensor to allow for different HW configuration btwn users
// todo add a 24h and a ALL Points and a View Current Record charts mode, selected w joystick up / down. 118KB sram free if 100 chart points --> 118.000 / 15 chart series / ~2Bytes = 3933 datasets. If points every 5' --> 327 hours  = 13.6 days
// todo allow screenSampleInterval > logInterval when logging not ON
// todo rationalize where if(sensorError) must be used. Remove from LOOP, add to each sensorMeasure sub ?
// todo Values & Charts: add before Multigas & SCD40 display code: if (!sensorerror) { proceed }
// todo rationalize use of fakesensordata: only if setupError on sensor. For errors after successful setup send back 0s, like in SCD40 code
// todo screen battery display (can't while recording on) instead of NOW
// todo fix cursors not available when in banner mode --> actually make white settings become grey
// todo issue cursor when toggling btwn banner and settings / battery: see cursor lagging. Because an UPDATESCREEN was not triggered ???
// todo no flickering screen on refresh. Confusing as happens on screenInterval (2s) which is NOT sampleInterval. No sample acquired
// bug  all SD files show a 12/31/2017 11pm date. Logged a github issue against seeed_arduino_fs library
// todo add RTC for correct time at power-on and sampling
// todo save session settings in FLASH
// todo add Alert thresholds to settings screen
// todo add a setting to stop datalogging when voltage < 3.1v, 3.2v...
// todo add a setting to stop operation and go to sleep when voltage < 3V to avoid dying when accessing sensors ?
// todo add a setting that Initiates Sen55 Cleaning
// todo add a setting that controls the chart data's nb of samples displayed ?
// todo add a setting for sensor names (most won't recompile to change them. Save to FLASH)
// todo add a setting for temp in Farenheit
// todo add setting for SCD40 altitude correction
// todo SCD40 disable Auto Self Calibration or it'll assume 400ppm once a week and start drifting, and add setting to Force Self Calibration.
...
// todo implement an error display system. Like SD Open File fail during logging, etc. Maybe error icon on banner flashing and an associated display screen when clicked on
// todo cleanup for readability in setup only call device1setup device2setup, etc, instead of doing it in line. Use True / False for call success and Error message or in the subs themselves ?
// todo clean up replace SCREEN_WIDTH/height() and SCREEN_WIDTH/SCREEN_HEIGHT w constants
// todo optimization: replace all drawstring(,,font) by a single proper setFreeFont + drawString(,) to reduce calls to setFont. Problem: (,,2) results in lean font, what's the setFreeFont equivalent ?
// todo optimization instead of padding time hh mm ss with call to padding functions, use inline code if hh<10 then txt +=0; txt += hh; like at end of drawBanner
// todo clean up DEBUG & REMOVE lines
// todo clean up: add description line for each sub inline with its declaration, like in "void screenMainSettings() { // display the list of settings and their values"
// todo bq27441's SOC estimate widely pessimistic, does not compensate even when shows depleted but still sees high voltage (~3.6v). Take several datalogging runs from FULL to EMPTY to track performance soc/V/I, then do TI's calibration per application note
// todo conditional statement for all serial.print in code when !Serial is detected in setup at boot

// HW SETUP
// Sen55 connects to SDA/SCL1 and / or SDA/SCL0, and requires 5V power
// Seeed Multigas, RTC, SCD40 connect in a daisy chain to WIO's bottom left Grove port (=SDA/SCL1)
// Enable '#define INTERNAL_RTC' below to use the WIO's RTC, or comment it out to use an external Adafruit RTC
...
const bool FAKE_SENSOR_DATA = false;    // set to true during development if need to get and display sensor values without requiring the sensors to be present
...
#define NB_CHART_POINTS 100                 // maximum number of data points to chart
...
doubles sen55ChartSeries[3][8];             // queue of double per line_chart library. Data points for the chart
#define MAXBUF_REQUIREMENT 48               // Sen55 commands use up to 48 bytes. On some Arduinos the default buffer space is not large enough
...
const String WIO_FIRMWARE = "Local Sensors 0.1";
...
const int BANNER_X1 = BANNER_X_START; // 11;
...
String settingsText[] = {
  ...
  "Screen sample interval (s):<99>",            // Screen Sample Interval must the 1st user modifiable setting in the list
...
void sen55PrintModuleVersions(int index) {
  uint16_t error;
  char errorMessage[256];
  unsigned char productName[32];
  uint8_t productNameSize = 32;
  error = sen55[index].getProductName(productName, productNameSize);
  if (error) {
    Serial.print("Error trying to execute getProductName() for sen55[");
    Serial.print(index);
    Serial.print("]: ");
    errorToString(error, errorMessage, 256);
    Serial.println(errorMessage);
  ...
  error = sen55[index].getVersion(firmwareMajor, firmwareMinor, firmwareDebug,
                            hardwareMajor, hardwareMinor, protocolMajor, protocolMinor);
  if (error) {
    Serial.print("Error trying to execute getVersion() for sen55[");
    Serial.print(index);
    Serial.print("]: ");
    errorToString(error, errorMessage, 256);
    Serial.println(errorMessage);
...
void sen55PrintSerialNumber(int index) {
  uint16_t error;
  char errorMessage[256];
  unsigned char serialNumber[32];
  uint8_t serialNumberSize = 32;
  error = sen55[index].getSerialNumber(serialNumber, serialNumberSize);
  if (error) {
    Serial.print("Error trying to execute getSerialNumber() for sen55[: ");
    Serial.print(index);
    Serial.print("]: ");
    errorToString(error, errorMessage, 256);
    Serial.println(errorMessage);
...
void sen55Setup(int index) {
  ...
  // Print SEN55 module information if i2c buffers are large enough
    ...
    sen55PrintSerialNumber(index);
    sen55PrintModuleVersions(index);
  #endif
...
void scd40Measure() { // measure CO2/T/RH. Requires at least 5 seconds btwn reads or SCD40 will respond with 0 and "data not ready"
  if (DEBUG_MODE) {
  ...
  if (!isDataReady) {
      Serial.println("Warning: SCD40 data not ready");        // SCD40 has a 5 second refractory window before new values. Receiving old values in the meantime
      return;
  }
...
void screenSnapshot() {
  if (sdCardMount()) {
    if (createBMPfile()) {
      writeBMPheader();
      writePixelsToSDcard_custom_565to888_conversion();
    }
  }
  flashBanner("stop");
}
...
void writePixelsToSDcard_custom_565to888_conversion() { 
  // Read each pixel line and write to SD card. Color accurate (almost), even for mixed r,g,b colors.
...
void convert565to888() {
  // convert each pixel's 16b (565) value to 24b (888). Color accurate.
...
void screenBattery() {
  ...
  if (socVal <= 100) {                                             // if can't find the bq27441 most values read 65535
    ...
    tempStr = String(tint) + ".";
    if (tdec > 9) {
      tempStr += String(tdec) + " C ";
    } else {
      tempStr += "0" + String(tdec) + " C ";
    }
  } else {
...
void flashBanner(String action) {
  if (action == "start") {
...
void setup() {
 ...
 // pinMode(SDCARD_DET_PIN, INPUT_PULLUP); // SD card detection switch

}

void loop() {
  millisNow = millis();
  // if (millisNow - millisOldSample < 0) {  // millis() rolls over every ~50 days
